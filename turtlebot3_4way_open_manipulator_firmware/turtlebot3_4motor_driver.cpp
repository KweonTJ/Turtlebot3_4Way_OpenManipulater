...
Turtlebot3_4Motor_Driver::Turtlebot3_4Motor_Driver()
: baudrate_(BAUDRATE),
  protocol_version_(PROTOCOL_VERSION),
  left_wheel_id_(DXL_LEFT_ID),
  right_wheel_id_(DXL_RIGHT_ID),
  left_front_wheel_id_(3),    // 추가 왼쪽 앞바퀴
  right_front_wheel_id_(4)    // 추가 오른쪽 앞바퀴 id 변경 가능
{
  torque_ = false;
...
bool Turtlebot3_4Motor_Driver::init(String turtlebot3)
{
...
bool Turtlebot3_4Motor_Driver::setTorque(bool onoff)
{
  uint8_t dxl_error = 0;
  ...
  // [추가] 왼쪽 앞바퀴 토크
dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, left_front_wheel_id_, ADDR_X_TORQUE_ENABLE, onoff, &dxl_error);
if(dxl_comm_result != COMM_SUCCESS)
{
  Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
  return false;
...
// [추가] 오른쪽 앞바퀴 토크
dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, right_front_wheel_id_, ADDR_X_TORQUE_ENABLE, onoff, &dxl_error);
if(dxl_comm_result != COMM_SUCCESS)
{
  Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
  return false;
...
bool Turtlebot3_4Motor_Driver::getTorque()
{
  return torque_;
}

void Turtlebot3_4Motor_Driver::close(void)
{
...
bool Turtlebot3_4Motor_Driver::readEncoder(int32_t &left_value, int32_t &right_value)
{
  ...
  dxl_addparam_result = groupSyncReadEncoder_->addParam(left_front_wheel_id_); // [추가]
  if (dxl_addparam_result != true) 
    return false;
  
  dxl_addparam_result = groupSyncReadEncoder_->addParam(right_front_wheel_id_); // [추가]
  if (dxl_addparam_result != true)
    return false;
  ...
  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(left_front_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION); // [추가]
  if (dxl_getdata_result != true)
    return false;
  
  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(right_front_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION); // [추가]
  if (dxl_getdata_result != true) 
    return false;
  ...
  // [수정] 4개 데이터 읽기
  int32_t left_rear_val  = groupSyncReadEncoder_->getData(left_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  int32_t right_rear_val = groupSyncReadEncoder_->getData(right_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  int32_t left_front_val = groupSyncReadEncoder_->getData(left_front_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  int32_t right_front_val= groupSyncReadEncoder_->getData(right_front_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);

  // [수정] 좌/우 평균값 반환
  left_value  = (left_rear_val + left_front_val) / 2;
  right_value = (right_rear_val + right_front_val) / 2;

  groupSyncReadEncoder_->clearParam();
  return true;
}

bool Turtlebot3_4Motor_Driver::writeVelocity(int64_t left_value, int64_t right_value)
{
  ...
  if (dxl_addparam_result != true)
    return false;
  dxl_addparam_result = groupSyncWriteVelocity_->addParam(left_front_wheel_id_, (uint8_t*)&left_data_byte); // 왼쪽 앞바퀴 추가
  if (dxl_addparam_result != true)
    return false;
  ...
  if (dxl_addparam_result != true)
    return false;
  dxl_addparam_result = groupSyncWriteVelocity_->addParam(right_front_wheel_id_, (uint8_t*)&right_data_byte); // 오른쪽 앞바퀴 추가
  if (dxl_addparam_result != true)
    return false;
...
bool Turtlebot3_4Motor_Driver::controlMotor(const float wheel_radius, const float wheel_separation, float* value)
{

...
// ================== Globals ==================
uint8_t g_ctable[CT_BYTES] = {0};
Turtlebot3_4Motor_Driver motor_driver;
OpenManipulatorDriver    manipulator_driver;

int64_t g_vel_limit = 330; // example X-series goal velocity limit (unit: tick/s)
float   g_joint_limit[4][2] = { {-2.7f, 2.7f}, {-2.7f, 2.7f}, {-2.7f, 2.7f}, {-2.7f, 2.7f} };
...
float constrainf_fast(float x, float lo, float hi) {
  if (x < lo) return lo;
  if (x > hi) return hi;
  return x;
}
...
void ct_init() {
  ...
  uint16_t model = 0xBEEF;
  uint16_t fwv   = 0x0202; // 2.0.2
  ct_write<uint16_t>(CT_MODEL_ID, model);
  ct_write<uint16_t>(CT_FW_VERSION, fwv);
  ct_write<uint8_t >(CT_ENABLE, 0);
  ct_write<uint8_t >(CT_ESTOP,  0);
  ct_write<uint32_t>(CT_LAST_CMD_MS, 0);
}

// ================== Hardware Init ===========
void tb3_hw_init() {
  // motor & manipulator
  motor_driver.init(String("WafflePi")); // or your model name
  motor_driver.setTorque(false);
  manipulator_driver.init(OM_JOINT_ID, OM_JOINT_COUNT, OM_GRIPPER_ID, OM_GRIPPER_COUNT);

  // zero targets
  ct_write<int32_t>(CT_WHEEL_TARGET_L, 0);
  ct_write<int32_t>(CT_WHEEL_TARGET_R, 0);
}

// ================== HW Updates ===============
void hw_update_wheels() {
  int32_t encL=0, encR=0;
  motor_driver.readEncoder(encL, encR);
  ct_write<int32_t>(CT_WHEEL_CUR_L, encL);
  ct_write<int32_t>(CT_WHEEL_CUR_R, encR);
}

void hw_update_imu() {
  // Fill with your IMU read (replace with actual API)
  float ax=0, ay=0, az=9.81f, gx=0, gy=0, gz=0;
  ct_write<float>(CT_IMU_AX, ax);
  ct_write<float>(CT_IMU_AY, ay);
  ct_write<float>(CT_IMU_AZ, az);
  ct_write<float>(CT_IMU_GX, gx);
  ct_write<float>(CT_IMU_GY, gy);
  ct_write<float>(CT_IMU_GZ, gz);
}

void hw_update_battery() {
  float v = 12.0f; // replace with ADC read
  ct_write<float>(CT_BATTERY_VOLT, v);
}

void hw_update_odometry(float dt) {
  // Minimal placeholder integration; replace with your wheel->odom kinematics
  float x = ct_read<float>(CT_ODOM_X);
  float y = ct_read<float>(CT_ODOM_Y);
  float th= ct_read<float>(CT_ODOM_TH);
  float vx = 0.0f, vw = 0.0f;
  // compute vx,vw from encoders or last command if needed
  ct_write<float>(CT_ODOM_X,  x + vx * dt * cosf(th));
  ct_write<float>(CT_ODOM_Y,  y + vx * dt * sinf(th));
  ct_write<float>(CT_ODOM_TH, th + vw * dt);
  ct_write<float>(CT_ODOM_VX, vx);
  ct_write<float>(CT_ODOM_VW, vw);
}

void hw_apply_track_velocity() {
  int64_t tgtL = (int64_t)ct_read<int32_t>(CT_WHEEL_TARGET_L);
  int64_t tgtR = (int64_t)ct_read<int32_t>(CT_WHEEL_TARGET_R);
  if (!g_enabled || g_estop) { tgtL = 0; tgtR = 0; }
  if (tgtL >  g_vel_limit) tgtL =  g_vel_limit;
  if (tgtL < -g_vel_limit) tgtL = -g_vel_limit;
  if (tgtR >  g_vel_limit) tgtR =  g_vel_limit;
  if (tgtR < -g_vel_limit) tgtR = -g_vel_limit;
  motor_driver.writeVelocity(tgtL, tgtR); // driver maps to 4 motors
}

void hw_apply_manipulator() {
  if (!g_enabled || g_estop) return;

  // 컨트롤테이블에서 목표각 읽기 (float→double 변환)
  double j[4];
  j[0] = (double)ct_read<float>(CT_JOINT_TARGET_1);
  j[1] = (double)ct_read<float>(CT_JOINT_TARGET_2);
  j[2] = (double)ct_read<float>(CT_JOINT_TARGET_3);
  j[3] = (double)ct_read<float>(CT_JOINT_TARGET_4);
  double g = (double)ct_read<float>(CT_GRIPPER_TARGET);
  double gp[1] = {g};

  // 프로파일(이동시간) 설정이 필요하면 한 번 호출
  // manipulator_driver.writeJointProfileControlParam(move_time_sec);
  // manipulator_driver.writeGripperProfileControlParam(move_time_sec);

  // 존재하는 API로 전송
  manipulator_driver.writeJointPosition(j);
  manipulator_driver.writeGripperPosition(gp);
}
...
void serial_send_status(uint8_t ok) {
  uint8_t pkt[7] = {PKT_HEAD1, PKT_HEAD2, (uint8_t)0x10, ok, 0, 0, 0}; // simple ACK
  uint16_t crc = crc16_x25(pkt, 5);
  pkt[5] = (uint8_t)(crc & 0xFF);
  pkt[6] = (uint8_t)(crc >> 8);
  Serial.write(pkt, sizeof(pkt));
}

void serial_send_read(uint16_t addr, uint16_t len) {
  if (len == 0 || !ct_ptr(addr) || (addr + len) > CT_BYTES) { serial_send_status(0); return; }
  uint8_t hdr[7] = {PKT_HEAD1, PKT_HEAD2, PKT_CMD_READ, (uint8_t)(addr & 0xFF), (uint8_t)(addr>>8), (uint8_t)(len & 0xFF), (uint8_t)(len>>8)};
  uint16_t crc1 = crc16_x25(hdr, sizeof(hdr));
  Serial.write(hdr, sizeof(hdr));
  Serial.write(&g_ctable[addr], len);
  uint16_t crc2 = crc16_x25(&g_ctable[addr], len);
  uint16_t crc  = crc16_x25((uint8_t*)&crc1, 2) ^ crc2; // optional; simplest: send separate CRCs
  Serial.write((uint8_t*)&crc, 2);
}

void handle_write(uint16_t addr, const uint8_t* payload, uint16_t len) {
  if (!ct_ptr(addr) || (addr + len) > CT_BYTES) { serial_send_status(0); return; }
  memcpy(&g_ctable[addr], payload, len);
  g_last_cmd_ms = millis();

  // side-effects
  if (addr <= CT_ENABLE && (addr+len) >= (CT_ENABLE+1)) {
    g_enabled = (ct_read<uint8_t>(CT_ENABLE) != 0);
    motor_driver.setTorque(g_enabled && !g_estop);
  }
  if (addr <= CT_ESTOP && (addr+len) >= (CT_ESTOP+1)) {
    g_estop = (ct_read<uint8_t>(CT_ESTOP) != 0);
    if (g_estop) motor_driver.writeVelocity(0,0);
  }
  if ((addr <= CT_WHEEL_TARGET_L && (addr+len) >= (CT_WHEEL_TARGET_R+4)) ||
      (addr == CT_WHEEL_TARGET_L) || (addr == CT_WHEEL_TARGET_R)) {
    hw_apply_track_velocity();
  }
  if ((addr <= CT_JOINT_TARGET_1 && (addr+len) >= (CT_GRIPPER_TARGET+4))) {
    hw_apply_manipulator();
  }
  serial_send_status(1);
}

void serial_process_rx() {
  while (Serial.available()) {
    uint8_t b = (uint8_t)Serial.read();
    if (rxi < 2) {
      if ((rxi==0 && b!=PKT_HEAD1) || (rxi==1 && b!=PKT_HEAD2)) { rxi=0; continue; }
    }
    rxbuf[rxi++] = b;
    if (rxi == sizeof(pkt_hdr_t)) {
      pkt_hdr_t* h = (pkt_hdr_t*)rxbuf;
      if (h->h1!=PKT_HEAD1 || h->h2!=PKT_HEAD2) { rxi=0; continue; }
      uint16_t need = sizeof(pkt_hdr_t) + h->len + 2; // +crc
      if (need > sizeof(rxbuf)) { rxi=0; continue; }
      // wait more
      while (Serial.available() && rxi < need) rxbuf[rxi++] = (uint8_t)Serial.read();
      if (rxi < need) return; // wait next loop

      uint16_t crc = crc16_x25(rxbuf, need-2);
      uint16_t rxcrc = rxbuf[need-2] | ((uint16_t)rxbuf[need-1]<<8);
      if (crc != rxcrc) { rxi=0; serial_send_status(0); continue; }

      uint16_t addr = h->addr;
      uint16_t len  = h->len;
      const uint8_t* payload = &rxbuf[sizeof(pkt_hdr_t)];

      switch (h->cmd) {
        case PKT_CMD_PING:  serial_send_status(1); break;
        case PKT_CMD_READ:  serial_send_read(addr, len); break;
        case PKT_CMD_WRITE: handle_write(addr, payload, len); break;
        default:            serial_send_status(0); break;
      }
      rxi = 0;
    }
  }
...
void loop() {
  static uint32_t t_imu=0, t_odom=0, t_bat=0, t_wd=0, t_wheel=0;
  uint32_t now = micros();

  serial_process_rx();

  // fast wheel feedback
  if ((uint32_t)(micros()-t_wheel) >= (1000000UL/LOOP_HZ_CTRL)) {
    t_wheel = micros();
    hw_update_wheels();
  }

  // IMU 200 Hz
  if ((uint32_t)(micros()-t_imu) >= (1000000UL/LOOP_HZ_IMU)) {
    t_imu = micros();
    hw_update_imu();
  }

  // Odom 30 Hz
  if ((uint32_t)(micros()-t_odom) >= (1000000UL/LOOP_HZ_ODOM)) {
    uint32_t nowu = micros();
    float dt = (nowu - t_odom) * 1e-6f;
    t_odom = nowu;
    hw_update_odometry(dt);
  }

  // Battery 10 Hz
  if ((uint32_t)(micros()-t_bat) >= (1000000UL/LOOP_HZ_BATTERY)) {
    t_bat = micros();
    hw_update_battery();
  }

  // Watchdog
  if ((uint32_t)(millis()-g_last_cmd_ms) > WATCHDOG_MS) {
  ...
  // housekeeping
  ct_write<uint32_t>(CT_LAST_CMD_MS, millis());
}

...
*******************************************************************************/
static Turtlebot3MotorDriver4WD motor_driver;
static OpenManipulatorDriver4WD manipulator_driver(motor_driver.getDxl());
...
*******************************************************************************/
static Turtlebot3Sensor4WD sensors;

/*******************************************************************************
* Declaration for diagnosis
*******************************************************************************/
static Turtlebot3Diagnosis4WD diagnosis;

/*******************************************************************************
* Declaration for controllers
*******************************************************************************/
static Turtlebot3Controller4WD controllers;
...
*******************************************************************************/
void TurtleBot3Core4WD::begin(const char* model_name)
{
  ...
  dxl_slave.addControlItem(ADDR_PRESENT_POSITION_L, control_items.present_position[MortorLocation::LEFT_REAR]);
  dxl_slave.addControlItem(ADDR_PRESENT_POSITION_R, control_items.present_position[MortorLocation::RIGHT_REAR]);
  dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_L, control_items.present_velocity[MortorLocation::LEFT_REAR]);
  dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_R, control_items.present_velocity[MortorLocation::RIGHT_REAR]);
  dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_L, control_items.present_current[MortorLocation::LEFT_REAR]);
  dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_R, control_items.present_current[MortorLocation::RIGHT_REAR]);
  ...
  dxl_slave.addControlItem(ADDR_PROFILE_ACC_L, control_items.profile_acceleration[MortorLocation::LEFT_REAR]);
  dxl_slave.addControlItem(ADDR_PROFILE_ACC_R, control_items.profile_acceleration[MortorLocation::RIGHT_REAR]);
...
void update_motor_status(uint32_t interval_ms)
{
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    pre_time_dxl = millis();
    if(get_connection_state_with_motors() == true){
      motor_driver.read_present_position(control_items.present_position[MortorLocation::LEFT_REAR], control_items.present_position[MortorLocation::RIGHT_REAR]);
      motor_driver.read_present_velocity(control_items.present_velocity[MortorLocation::LEFT_REAR], control_items.present_velocity[MortorLocation::RIGHT_REAR]);
      if(motor_driver.read_present_current(current_l, current_r) == true){
        control_items.present_current[MortorLocation::LEFT_REAR] = current_l;
        control_items.present_current[MortorLocation::RIGHT_REAR] = current_r;
      }
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    case ADDR_PROFILE_ACC_R:
      if(get_connection_state_with_motors() == true)
        motor_driver.write_profile_acceleration(control_items.profile_acceleration[MortorLocation::LEFT_REAR], control_items.profile_acceleration[MortorLocation::RIGHT_REAR]);
      break;
...
void test_motors_with_buttons(uint8_t buttons)
{
  ...
  if(get_connection_state_with_motors() == true){
    motor_driver.read_present_position(current_tick[MortorLocation::LEFT_REAR], current_tick[MortorLocation::RIGHT_REAR]);
  }

  if (buttons & (1<<0))  
  {
    move[VelocityType::LINEAR] = true;
    saved_tick[MortorLocation::RIGHT_REAR] = current_tick[MortorLocation::RIGHT_REAR];
  ...
  {    
    if (abs(saved_tick[MortorLocation::RIGHT_REAR] - current_tick[MortorLocation::RIGHT_REAR]) <= diff_encoder)
    {
